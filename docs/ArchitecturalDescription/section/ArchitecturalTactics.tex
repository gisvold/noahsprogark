...
Data transfer has to be set to a minimum to reduce the impact of delay between clients and server. The clients itself wouldn't have much load while its operative, altough we do have to keep in mind that the client needs an active connection to the server when playing. Most of our challenge will therefore be on the server side. Mostly by handling large amount of users and network loads. 
